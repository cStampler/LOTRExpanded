...
public class DRStructureArthedainVillage extends LOTRWorldGenStructureBase2 {
    private static int VILLAGE_SIZE = 64;

    public DRStructureArthedainVillage(boolean flag) {
        super(flag);
    }
    ...
    public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
        ...
        for(l = 0; l < chiefainHouses; ++l) {
            structure = new DRStructureArthedainCaptain(this.notifyChanges);
            this.attemptHouseSpawn(structure, world, random);
        }
        for(l = 0; l < houses; ++l) {
            structure = new DRStructureArthedainHouse(this.notifyChanges);
            this.attemptHouseSpawn(structure, world, random);
        }
        for(l = 0; l < smithhouses; ++l) {
            structure = new DRStructureArthedainSmith(this.notifyChanges);
            this.attemptHouseSpawn(structure, world, random);
        }
        return true;
    }
